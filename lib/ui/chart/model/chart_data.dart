...
abstract class BaseChart {
  ...
  BaseChart(this.time, this.value);
}

class OffsetTimelineConst {
  static Offset initEndOrStart<T extends BaseChart>(double width, double height, T data,
      bool dataMoreThanHeight, int maxData) {
    ...
    var deviant = dataMoreThanHeight ? maxData / height : 1;
    var errorOffset = height * deviant;
    var milestone = height / (errorOffset / value);
    var offsetY = dataMoreThanHeight ? height - milestone : height - milestone;
    return Offset(offsetX, offsetY);
  }

  static Point initPoint<T extends BaseChart>(
      double width,
      double height,
      T meals,
      bool mealMoreThanHeight,
      int maxMeal,
      ) {
    var dtStart = DateTime.parse(meals.time);
    var hour = dtStart.hour;
    var minutes = dtStart.minute;
    var timeInMinute = 60 / minutes;
    var timeInHour = 1 / timeInMinute;
    var separateTime = hour + timeInHour;
    var offsetX = (width / (24 / separateTime));
    var value = meals.value;
    var deviant = mealMoreThanHeight ? maxMeal / height : 1;
    var errorOffset = height * deviant;
    var milestone = height / (errorOffset / value);
    var offsetY = mealMoreThanHeight ? height - milestone : height - milestone;
